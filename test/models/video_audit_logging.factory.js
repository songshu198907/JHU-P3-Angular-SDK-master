(function(module, inject, angular) {
  ...
  describe("Model Service - $videoAuditLogging", function() {
    var model, config;
    ...
    beforeEach(inject(function($injector) {
      ...
      model = $injector.get("$videoAuditLogging");
    ...
    it("should be able to create an instance", function() {
      var instance = model.create({
        ...
        watched_entire_video: undefined,
    ...
    describe("Query Scope - export_video_log", function() {
      it("should exist", function() {
        expect(model).to.respondTo("export_video_log");
      });

      
      it("should return an array of instances", function() {
        var result = model.export_video_log();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.export_video_log({}, successSpy);
      });
    });
    
    describe("Query Scope - by_video_and_user_id", function() {
      it("should exist", function() {
        expect(model).to.respondTo("by_video_and_user_id");
      });

      
      it("should return an array of instances", function() {
        var result = model.by_video_and_user_id();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.by_video_and_user_id({}, successSpy);
      });
    });
