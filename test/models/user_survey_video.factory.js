(function(module, inject, angular) {
  ...
  describe("Model Service - $userSurveyVideo", function() {
    var model, config;
    ...
    beforeEach(inject(function($injector) {
      ...
      model = $injector.get("$userSurveyVideo");
    ...
    it("should be able to create an instance", function() {
      var instance = model.create({
        ...
        video_number: undefined,
          
        
        video_position: undefined,
    ...
    describe("Query Scope - get_videos_for_user_survey_id", function() {
      it("should exist", function() {
        expect(model).to.respondTo("get_videos_for_user_survey_id");
      });

      
      it("should return an array of instances", function() {
        var result = model.get_videos_for_user_survey_id();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.get_videos_for_user_survey_id({}, successSpy);
      });
    });
    
    describe("Query Scope - get_video_by_id", function() {
      it("should exist", function() {
        expect(model).to.respondTo("get_video_by_id");
      });

      
      it("should return an array of instances", function() {
        var result = model.get_video_by_id();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.get_video_by_id({}, successSpy);
      });
    });
