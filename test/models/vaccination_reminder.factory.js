(function(module, inject, angular) {
  ...
  describe("Model Service - $vaccinationReminder", function() {
    var model, config;
    ...
    beforeEach(inject(function($injector) {
      ...
      model = $injector.get("$vaccinationReminder");
    ...
    it("should be able to create an instance", function() {
      var instance = model.create({
        ...
        trigger_days_from_dob: undefined,
    ...
    describe("Query Scope - sorted_by_trigger_days", function() {
      it("should exist", function() {
        expect(model).to.respondTo("sorted_by_trigger_days");
      });

      
      it("should return an array of instances", function() {
        var result = model.sorted_by_trigger_days();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.sorted_by_trigger_days({}, successSpy);
      });
    });
    
    describe("Query Scope - send_vaccination_reminders", function() {
      it("should exist", function() {
        expect(model).to.respondTo("send_vaccination_reminders");
      });

      
      it("should return an array of instances", function() {
        var result = model.send_vaccination_reminders();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.send_vaccination_reminders({}, successSpy);
      });
    });
