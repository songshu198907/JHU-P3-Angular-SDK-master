(function(module, inject, angular) {
  ...
  describe("Model Service - $surveyQuestionSkipLogic", function() {
    var model, config;
    ...
    beforeEach(inject(function($injector) {
      ...
      model = $injector.get("$surveyQuestionSkipLogic");
    ...
    describe("Query Scope - get_skip_logic_by_answers", function() {
      it("should exist", function() {
        expect(model).to.respondTo("get_skip_logic_by_answers");
      });

      
      it("should return an array of instances", function() {
        var result = model.get_skip_logic_by_answers();
        expect(result).to.be.an.instanceof(Array);
        expect(result.length).to.be.at.least(1);
        expect(result[0].id).to.exist;
      });
      ...
      it("should accept success callbacks", function() {
        ...
        model.get_skip_logic_by_answers({}, successSpy);
      });
    });
